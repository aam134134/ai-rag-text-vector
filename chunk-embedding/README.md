# Create Embeds for Chunked Data

## To Build and Run via Podman/Docker

- Build and run: ```podman compose up --build```
- Drop a JSONL (*.jsonl) file to ingest into ```../data/ingest/chunks```
- View embeds in ```../data/chunks/embeds```
...
- Run program: ```python embed_chunks.py```
- Drop a JSONL (*.jsonl) file to ingest into ```../data/ingest/chunks```
- View embeds in ```../data/chunks/embeds```
